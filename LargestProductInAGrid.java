public class LargestProductInAGrid
{
    ...
    public LargestProductInAGrid()
    {
        ...
        for(int i = 0; i < grid.length; i++)
        {
            ...
            {
                if(x <= 16 && grid[i][x] *  grid[i][x + 1] *  grid[i][x + 2] *  grid[i][x + 3] > product)
                product = grid[i][x] *  grid[i][x + 1] *  grid[i][x + 2] *  grid[i][x + 3];
                if(i <= 16 && grid[i][x] *  grid[i + 1][x] *  grid[i + 2][x] *  grid[i + 3][x] > product)
                product = grid[i][x] *  grid[i+1][x] *  grid[i + 2][x] *  grid[i + 3][x];
                if(i <= 16 && x <= 16 && grid[i][x] *  grid[i + 1][x + 1] *  grid[i + 2][x + 2] *  grid[i + 3][x + 3] > product)
                product = grid[i][x] *  grid[i + 1][x + 1] *  grid[i + 2][x + 2] *  grid[i + 3][x + 3];
                if(i <= 16 && x >= 3 && grid[i][x] * grid[i + 1][x - 1] *  grid[i + 2][x - 2] *  grid[i + 3][x - 3] > product)
                product = grid[i][x] * grid[i + 1][x - 1] *  grid[i + 2][x - 2] *  grid[i + 3][x - 3];
            }
        }
